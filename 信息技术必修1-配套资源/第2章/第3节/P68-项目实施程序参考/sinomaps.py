...
class RGLight:
    ...
    def draw_lights(self, colors):
        try:
            self.light.clear()
            self.light.goto(0, 100)
            self.light.color('#000000')
            self.light.write('模拟交互式人形过街红绿灯', align='center', font=('Microsoft Yahei', 10, 'normal'))
    ...
    def draw_button(self):
        self._draw_rect(self.button, 0, 0, 30, 30, '#666666')
        self.button.color('#000000')
        self.button.goto(0, -10)
    ...
    def _button_click(self, x, y):
        if not self.buttonEnabled:
            return
        if (x < -10 and y < -10) or (x < -10 and y > 10) or (x > 10 and y < -10) or (x > 10 and y > 10):
            return
    ...
    def write_seconds(self, second, color):
        try:
            ...
            self.secondLight.write(str(second).zfill(2), align='center', font=("Microsoft Yahei", 15, "normal"))
        except tkinter.TclError:
            self.running = False
